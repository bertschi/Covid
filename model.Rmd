...
data {
  int<lower=0> T;        // number of observations
  int<lower=0> cases[T]; // observed number of cases (cumulative total)
  int<lower=1> N;        // population size
  int<lower=1> tau_max;  // maximum window length into past
}
parameters {
  real<lower=0> beta;
  // Is tauE actually identifiable?
  // real<lower=0, upper=tau_max> tauE;
  // real<lower=0> tempE;
  // real<lower=tauE, upper=tau_max> tauI;
  real<lower=0, upper=tau_max> tauI;
  real<lower=0> tempI;
  real<lower=0, upper=1> pobs; // Currently not used ...
  real<lower=0> delta;     // Over- or underdispersion of gamma distribution
  real<lower=0> deltaobs;
  real<lower=0> deltaI[T]; // latent number of new cases per day
}
model {
  beta ~ normal(0, 1);
  // tauE ~ normal(3, 2);
  ...
  pobs ~ beta(10, 10);
  delta ~ normal(0, 10);
  deltaobs ~ normal(0, 10);
  
  for (t in 1:(T-1)) {
    real S = N - sum(deltaI[1:(t-1)]);
    real E; real I; real R;

    E = 0;
    // for (tau in 1:(t+tau_max-1)) {
    //   real w_tau = (1 - inv_logit((tau - tauE) / tempE));
    //   if (t - tau > 0)
	// E += w_tau * deltaI[t - tau];
    //   else
	// E += w_tau * deltaI[1];
    // }
    ...
    // Want to match beta-binomial with mean S * beta * I / N and overdispersion delta
    {
      real p = 1 - exp(- beta * I / N);  // Explanation of formula above
      real mu = S * p;
      real sig2 = S * p * (1 - p) * delta;
      deltaI[t+1] ~ gamma(mu^2 / sig2, mu / sig2);

      // Note: Should probably assume that infected are observed with some delay ...
      // cases[t+1] - cases[t] ~ neg_binomial_2(pobs * deltaI[t+1], deltaobs);
      // Try this ...
      {
	...
	for (tau in 1:(t-1)) {
	  real w_tau = (1 - inv_logit((tau - tauI) / tempI));
	  Iobs -= w_tau * cases[t - tau];
	}
	if (Iobs < 0)
	  Iobs = 0;
	cases[t+1] - cases[t] ~ neg_binomial_2((1 - exp(- pobs)) * Iobs, deltaobs);
      }
    }
  }
}
